...
impl<'a> ExpressionInferrer<'a> {
    ...
    pub(super) fn infer_expression_type_with_expected(
        ...
        let ty = match syntax_node.kind() {
            ...
            SyntaxNodeKind::StructLiteral => self.infer_struct_literal_type(node, None),
            ...
            SyntaxNodeKind::InferredStructLiteral => {
                ...
                if let Some(target) = self.graph.syntax().child(node, 0) {
                    return self.infer_expression_type(target);
                }
            ...
            SyntaxNodeKind::PathExpression => self.infer_path_variant_expression_type(node, None),
    ...
    fn infer_new_array_expression_type(&mut self, node: NodeId) -> Option<TypeId> {
        ...
        if let Some(TypeKind::Array { element }) = self.layer.table().kind(ty) {
            // Temporary fix, is_float_type does not exist
            if !self.is_integer_type(*element) && !self.is_numeric_type(*element) {
                return None;
            }
        }
