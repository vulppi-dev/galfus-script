...
impl<'a> DeclarationTypeChecker<'a> {
    ...
    fn infer_binary_expression_type(&mut self, node: NodeId) -> Option<TypeId> {
    ...
    fn infer_unary_expression_type(&mut self, node: NodeId) -> Option<TypeId> {
    ...
    fn report_unsupported_operator(&mut self, operator: NodeId, operator_text: &str) {
        let span = self
            .graph
            .syntax()
            .node(operator)
    ...
    fn report_unary_operator_type_error(
        &mut self,
        operator: NodeId,
        expected: &str,
        operand: TypeId,
